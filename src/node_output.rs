...
pub struct NodeOutput {
    ui_callback: Box<dyn FnOnce(&mut Ui)>,
    output_callback: Box<dyn FnOnce() -> Box<dyn Any>>,
    output_type: TypeId,
}
...
impl NodeOutput {
    ...
    pub fn new<T: 'static>(
        ui_callback: impl FnOnce(&mut Ui) + 'static,
        output_callback: impl FnOnce() -> T + 'static,
    ) -> Self {
        Self {
    ...
    pub fn ui(ui_callback: impl FnOnce(&mut Ui) + 'static) -> Self {
        Self {
            ui_callback: Box::new(ui_callback),
    ...
    pub fn output<T: 'static>(output_callback: impl FnOnce() -> T + 'static) -> Self {
        Self {
